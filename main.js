...
const addItem = () => {
  ...
  if (multiText !== "")
  {
    ...
    items = arrayMultiText.filter(item => item !== "")
...
const getRandomItem = () => {
  ...
  let item = ""
  let lisHtml = document.getElementById("item-list").getElementsByClassName("not-marked")

  if (uniqueFlag) {
    if (uniqueItems.length === 0) {
      itemResult.innerText = 'Inga fler unika värden från listan';

      return
    }

    let index = Math.floor(Math.random() * uniqueItems.length)
    item = uniqueItems[index];
    itemResult.innerText = item;
    let markedItem = lisHtml.item(index)
    markedItem.classList.add('marked')
    markedItem.classList.remove('not-marked')
    // lis.forEach(li => li.classList.add('marked'))

    // Remove number from uniqueItems
...
const uniqueRandom = () => {
  ...
  // Create list of uniqueItems
  if (uniqueFlag) {
    uniqueItems = [...items]
  }

  // Clear list of uniqueItems
  if (!uniqueFlag) {
    uniqueItems = []
    itemResult.innerText = '';

    let lisHtml = document.getElementById("item-list").getElementsByTagName("li")

    for (let index = 0; index < lisHtml.length; index++) {
      let markedItem = lisHtml.item(index)
      markedItem.classList.remove('marked')
      markedItem.classList.add('not-marked')
    }
  }
}
